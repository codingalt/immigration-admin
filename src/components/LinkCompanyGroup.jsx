...
import { useApproveGroupClientPhase1Mutation, useGetGroupClientAppByIdQuery, useLinkGroupCompanyMutation } from "../services/api/companyClient";

const LinkCompanyGroup = () => {
  ...
  const handleApprove = async () => {
    await approveGroupClientPhase1({ applicationId: applicationId });
  };
  ...
  const [linkGroupCompany, resultCompany] = useLinkGroupCompanyMutation();
  const {
  ...
  const handleLinkCompany = async (values) => {
    ...
    await linkGroupCompany({ applicationId: applicationId, data: newObj });
  };
...
export default LinkCompanyGroup;
