...
import { useFinalConfirmationMutation } from "../services/api/applicationApi";
...
const FinalConfirmationModal = ({
  ...
  const pdfRef = useRef();
  const [finalConfirmationMutat, res] = useFinalConfirmationMutation();
  ...
  const handleSubmit = async (approve) => {
    ...
    if (approve) {
      ...
      await finalConfirmationMutat({formData: formData, applicationId: applicationId});
